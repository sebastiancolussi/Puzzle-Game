...
var instrucciones = ["Usar las flechas para mover las piezas.", "Ordenar las piezas hasta alcanzar la imagen objetivo."];
...
function mostrarInstrucciones(instrucciones) {
  ...
  for (var i = 0; i < instrucciones.length; i++) {
    // var recorrido = instrucciones;
    mostrarInstruccionEnLista(instrucciones[i], 'lista-instrucciones')
  }
  // Tambien se puede hacer de esta forma sin usar la funcion dada en la parte de abajo.

  // var ul = document.getElementById("lista-instrucciones");

  // var il = document.createElement("li");
  // ul.appendChild(il); 
  // il.innerHTML = recorrido[0]; 

  // var il2 = document.createElement("li");
  // ul.appendChild(il2); 
  // il2.innerHTML = recorrido[1];
}
...
function chequearSiGano() {
  ...
  for (var i = 0; i < grilla.length; i++) {
    for (var j = 0; j < grilla.length; j++) {
      if (grillaGanadora[i][j] !== grilla[i][j] || grillaGanadora.length !== grilla.length) {
        return false;
      }
    }
...
function intercambiarPosicionesGrilla(filaPos1, columnaPos1, filaPos2, columnaPos2) {
  //COMPLETAR
  var posInicial = grilla[filaPos1][columnaPos1];
  grilla[filaPos1][columnaPos1] = grilla[filaPos2][columnaPos2];
  grilla[filaPos2][columnaPos2] = posInicial;
}
